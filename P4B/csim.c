...
typedef struct cache_line {
    ...
    int statusLRU; // status bit to keep track of LRU
} cache_line_t;
...
void init_cache() {
    ...
    B = pow(2, b); // compute # of bytes in a block
    S = pow(2, s); // compute # of sets
...
void access_data(mem_addr_t addr) {
    ...
    // Get set index
    mem_addr_t setIndex = (addr >> b) & (S - 1);
    cache_set_t set = cache[setIndex];

    // Get tag
    mem_addr_t tag = addr >> (b + s);

    // Find most recently used line
    for (int i = 0; i < E; i++) {
        if (set[i].statusLRU > max && set[i].valid == 1) {
            max = set[i].statusLRU;
        }
    }
    ...
    // Evict
    for (int i = 0; i < E; i++) {  // Find least recently used line
        if (min > set[i].statusLRU && set[i].valid == 1) {
            min = set[i].statusLRU;
            evictIndex = i;
        }
    }
    ...
    set[evictIndex].statusLRU = max + 1;
    miss_cnt++;
    evict_cnt++;
